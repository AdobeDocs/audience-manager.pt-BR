...
>[!TIP]
>
>Para obter as definições e descrições dessas configurações de [!UICONTROL Merge Rule], consulte [Opções de regra de mesclagem de Perfis definidas](merge-rule-definitions.md).

## Definição de metas do dispositivo {#device-personalization}

Esse cenário se aplica aos comerciantes que desejam avaliar um único perfil de dispositivo para um segmento de audiência definido no Audience Manager, a fim de fornecer uma experiência consistente ao dispositivo usando plataformas de definição de metas que suportam IDs de dispositivo (DSP, plataformas de personalização no site e outras plataformas de definição de metas baseadas em dispositivo), sem levar em conta a autenticação do usuário.
...
Ao usar a regra **[!UICONTROL No Cross-Device Profile]** + **[!UICONTROL Device Profile]**, [!DNL Device 1] e [!DNL Device 3] qualificam-se para o segmento, enquanto o Dispositivo 2 é ignorado.
...
A equipe de marketing da loja pode usar a regra **[!UICONTROL Current Authenticated Profiles]** + **[!UICONTROL No Device Profile]** para público alvo de negócios específicos entre John e Jane, com base apenas em sua atividade autenticada.
...
## Definição de metas online/offline {#device-household-targeting}
...
Este cenário de definição de metas está disponível somente para clientes que compraram o complemento [!DNL People-Based Destinations]. Essa regra permite que os comerciantes cheguem aos clientes com base em seus próprios dados autenticados.

Digamos que um varejista online queira alcançar os clientes existentes através de plataformas sociais e mostrar-lhes ofertas personalizadas com base em seus pedidos anteriores. Com [!UICONTROL People-Based Destinations], eles podem assimilar endereços de email com hash de seus próprios [!DNL CRM] no Audience Manager, criar segmentos a partir dos dados offline e enviar esses segmentos para as plataformas sociais nas quais desejam fazer publicidade, usando esse identificador com hash, otimizando seus gastos com publicidade.

Para saber mais sobre essa opção, consulte [Destinos baseados em pessoas](../destinations/people-based-destinations-overview.md).

![dispositivo completo](assets/all-cross-device.png)

## Opções de Gráfico de Dispositivos {#device-graph-options}

A escolha de uma opção [!UICONTROL device graph] para uma regra [!UICONTROL Profile Merge] depende de condições exclusivas de suas propriedades digitais e objetivos de negócios. Essas orientações gerais podem ajudá-lo a entender quando usar um tipo de gráfico em vez de outro. Observe que você deve ser um membro do [Adobe Experience Cloud Device Co-op](https://docs.adobe.com/content/help/pt-BR/device-co-op/using/home.html) ou ter uma relação contratual com um gráfico de dispositivo externo para usar essas opções. Consulte a tabela abaixo para obter orientações gerais sobre quando escolher uma opção de gráfico de dispositivo. Para casos de uso específicos, consulte [Casos de uso do gráfico de dispositivo de link de Perfil](profile-link-use-case.md) e [Casos de uso do gráfico de dispositivo externo](external-graph-use-cases.md).
...
<table id="table_66D9152D4FF040A186003272D456625D"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><span class="wintitle"> As </span> Mergerules do perfil criadas com o  <span class="wintitle"> Perfil </span> Linkoption são ideais para: </p> <p> 
     <ul id="ul_FF44FA894BB2448887C8EDA9C8407EF9"> 
      <li id="li_E22505210C664FE6A9AA7C61244B36DA">Propriedades digitais com um alto nível de autenticação de cliente. </li> 
      <li id="li_BE7112EE611E4DEB95B5C0A2852BFA97">Campanhas focadas e de baixo alcance. O gráfico do dispositivo <span class="wintitle"> Link do Perfil</span> é baseado apenas em dados determinísticos. Esse pool de perfis do dispositivo sempre será menor em relação ao pool de usuários e dispositivos não autenticados. </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p><span class="wintitle"> As </span> Mergerules do perfil com o  <a href="https://docs.adobe.com/content/help/en/device-co-op/using/about/overview.html" format="https" scope="external"> Experience Cloud Device Co-</a> opor qualquer gráfico de dispositivos externos integrado ao  <span class="keyword"> Audiência </span> Manager são ideais para: </p> <p> 
     ...
     </ul> </p> <p> <p>Dica: O <span class="keyword"> Device Co-op</span> é a sua melhor opção se você for um <span class="keyword"> Experience Cloud</span> cliente com baixa autenticação e sem relacionamento com qualquer provedor de gráficos de dispositivos. </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
