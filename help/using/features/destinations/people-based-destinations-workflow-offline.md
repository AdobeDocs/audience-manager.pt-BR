...
Independentemente de suas IDs de cliente Audience Manager existentes ([DPUUIDs](../../reference/ids-in-aam.md)) serem ou não emails com hash, você deve executar a característica de integração em relação à fonte de dados que contém seus [DPUUIDs](../../reference/ids-in-aam.md).
...
## Etapa 2 - Configurar as configurações da fonte de dados {#configure-data-source-settings}
...
>[!IMPORTANT]
>
> Esta etapa se aplica somente ao [Cenário 2](people-based-destinations-workflow-offline.md#configure-data-source-settings) descrito acima. Se os seus [DPUUIDs](../../reference/ids-in-aam.md) existentes já tiverem endereços de email com hash, pule para [Etapa 4 - Criar uma regra de mesclagem de Perfil para segmentação](#create-profile-merge-rule).

Digamos que você queira corresponder aos seus [DPUUIDs](../../reference/ids-in-aam.md) existentes do exemplo na Etapa 1 aos endereços de email com hash da tabela abaixo (coluna direita) e armazenar os endereços de email com hash na nova fonte de dados que você criou em [Etapa 2 - Configurar configurações da fonte de dados](#configure-data-source-settings).
...
5. Na lista **[!UICONTROL Cross-Device Profile Options]**, selecione a fonte de dados em que suas características estão integradas.
...
1. Na seção **[!UICONTROL Segment Mappings]**, selecione os segmentos que deseja enviar para esse destino. Esses seriam os segmentos criados na [Etapa 5 - Criar segmentos de Audiência](people-based-destinations-workflow-offline.md#create-audience-segments).
1. Salve o destino.
