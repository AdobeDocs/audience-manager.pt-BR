...
>[!IMPORTANT]
>
> Antes de continuar, verifique se a atividade do cliente que você está prestes a integrar já está definida no Audience Manager com o correspondente [características integradas](../traits/trait-and-segment-qualification-reference.md).

Independentemente de suas IDs de cliente do Audience Manager existentes ([DPUUIDs](../../reference/ids-in-aam.md)) forem emails com hash ou não, você deverá executar a integração de característica na fonte de dados que contém sua [DPUUIDs](../../reference/ids-in-aam.md).

### Exemplo

Você deseja qualificar as IDs do cliente da tabela abaixo para as IDs de característica integradas correspondentes. Vamos considerar que o seu [DPUUIDs](../../reference/ids-in-aam.md) são armazenados em uma fonte de dados com a ID 999999 e a ID da fonte de dados do Audience Manager é 123.
...
>[!IMPORTANT]
>
> Esta etapa se aplica somente a [Cenário 2](people-based-destinations-workflow-offline.md#configure-data-source-settings) acima descritas. Se o seu existente [DPUUIDs](../../reference/ids-in-aam.md) já são endereços de email com hash, saltar para [Etapa 4 - Criar uma regra de mesclagem de perfis para segmentação](#create-profile-merge-rule).

Digamos que você queira corresponder ao seu existente [DPUUIDs](../../reference/ids-in-aam.md) do exemplo na Etapa 1 para os endereços de email com hash da tabela abaixo (coluna à direita) e armazene os endereços de email com hash na nova fonte de dados criada em [Etapa 2 - Definir configurações da fonte de dados](#configure-data-source-settings).
...
| DPUUIDs (IDs de CRM) | Seu endereço de email | Endereço de email com hash |
| -------------------------------------- | --------------------- | ---------------------------------------------------------------- |
...
4. No **[!UICONTROL Profile Merge Rule Setup]** , selecione a **[!UICONTROL All Cross-Device Profiles]** regra do **[!UICONTROL Cross-Device Options]** lista.
5. No **[!UICONTROL Cross-Device Profile Options]** selecione a fonte de dados na qual suas características são integradas.
   ![merge-rule-setup](assets/pbd-pmr.png)

## Etapa 5 - Criar segmentos de público-alvo {#create-audience-segments}

Para criar novos segmentos a partir de dados somente offline, use o [Construtor de segmentos](../segments/segment-builder.md) e certifique-se de usar a nova regra de mesclagem de perfis criada na etapa anterior ao criar o segmento.
...
1. No **[!UICONTROL Segment Mappings]** selecione os segmentos que deseja enviar para esse destino. Esses seriam os segmentos criados em [Etapa 5 - Criar segmentos de público-alvo](people-based-destinations-workflow-offline.md#create-audience-segments).
1. Salve o destino.
