...
A variável [!UICONTROL Marketplace] é onde os compradores de dados vão para pesquisar e assinar feeds de dados públicos e privados. Siga estas etapas para assinar um feed de dados público.
...
1. Clique no nome do feed de dados que deseja usar. Isso abre o [página de detalhes do plano](../../../features/audience-marketplace/marketplace-data-buyers/marketplace-manage-subscriptions.md#marketplace-buyer-details) para o feed selecionado.

   ![detalhes do plano](assets/plan-details.png)

1. Escolha um caso de uso na tabela de assinaturas e:
   * Mova as **[!UICONTROL Subscription]** controle deslizante para **[!UICONTROL On]**.
   * Clique em **[!UICONTROL Review & Subscribe]**. Isso abre o [!UICONTROL Terms and Conditions] janela.

   ![assinar](assets/subscribe3.png)

1. No [!UICONTROL Terms and Conditions] janela:

   * **Importante:** Deixe a **[!UICONTROL ID sync]** caixa de seleção marcada. Esta configuração ajuda a melhorar as taxas de correspondência com seu provedor de dados.
   * Marque a caixa termos e condições e clique em **[!UICONTROL Accept]** para concluir o processo de assinatura.
...
* Verifique a assinatura verificando seu [!UICONTROL Traits] pasta. Consulte [Armazenamento para feeds de dados inscritos](../../../features/audience-marketplace/marketplace-data-buyers/marketplace-manage-subscriptions.md#find-subscribed-data-fee).

* Revise a documentação de faturamento e pagamento. Consulte os links relacionados abaixo.

### Práticas recomendadas {#best-practices}

Este é um conjunto de práticas recomendadas que recomendamos que você siga ao trabalhar com a [!UICONTROL Audience Marketplace]:
...
1. Não há garantia de que haverá uma sobreposição entre seus conjuntos de dados. Para que uma sobreposição seja válida, um usuário do conjunto de dados do cliente deve ser associado ao conjunto de dados do provedor de dados durante o período do relatório. Se os dados de mídia do cliente não forem fornecidos aos usuários no conjunto de dados do provedor, nunca haverá uma sobreposição.
...
Os compradores se inscrevem em feeds de dados e planos privados no **[!UICONTROL Audience Marketplace > Marketplace]**.
...
1. Clique no nome do feed de dados na [!UICONTROL Marketplace].
1. Clique em **[!UICONTROL Request Access]**. Isso abre a caixa de diálogo de solicitação.
1. Na caixa de diálogo de solicitação, escreva para o provedor uma nota expressando seu interesse no feed de dados e clique em **[!UICONTROL Send]**. O vendedor revisará sua mensagem e aprovará ou rejeitará sua solicitação. Enquanto aguarda por aprovação, &quot;Solicitado&quot; é exibido na janela [!UICONTROL Marketplace] para esse feed de dados.

   * **[!UICONTROL Request approved]**: O status na variável [!UICONTROL Marketplace] listar as alterações feitas em &quot;Acesso concedido&quot; e você receberá uma notificação automática. Nesse ponto, é possível assinar o feed. Consulte [Assinar um feed de dados público](../../../features/audience-marketplace/marketplace-data-buyers/marketplace-manage-subscriptions.md#subscript-public-data-feed) para obter instruções.
   * **[!UICONTROL Request denied]**: O texto &quot;Solicitado&quot; é removido da janela [!UICONTROL Marketplace] para o feed. Você pode tentar se inscrever novamente ou escolher um feed diferente.

## Descontos de feed de dados para compradores {#buyer-discount}

Entrada [!UICONTROL Audience Marketplace], os provedores podem oferecer aos compradores um desconto sobre o preço publicado de um [!DNL CPM] ou feed de dados de taxa fixa. No entanto, os valores de desconto não são visíveis para compradores no [!DNL Marketplace] lista de feeds. Mas você também pode solicitar um desconto ao assinar um feed de dados privado ou ao solicitar mais informações sobre um feed específico.
...
<table id="table_3C6E58F593BA48EC89ACBD9A26E4E74F"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Se você já estiver inscrito em um feed de dados privado e quiser solicitar um desconto: </p> 
    <ol id="ol_A58D419EBB9349E9B1225202535130F6"> 
     ...
     <li id="li_B1B5AA6F6CC64512A02D5E8861A5F266">Se o provedor conceder a você um desconto, inscreva-se novamente no feed no 1<sup>st</sup> dia do mês seguinte. </li> 
    </ol> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b>Novos assinantes do feed de dados privado</b> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>A <a href="../../../features/audience-marketplace/marketplace-private-feeds.md"> assinante potencial</a> é um comprador de dados que solicitou acesso a um feed de dados privado, recebeu aprovação do vendedor, mas não assinou o feed. Para solicitar um desconto como assinante em potencial: </p> 
    <ol id="ol_9CECDA92E7894B20AC8A777D78962188"> 
     <li id="li_618B64160CF24549AFCA73E006DCA35A">Ir para <b><span class="uicontrol"> Audience Marketplace &gt; Marketplace</span></b>. </li> 
...
1. Olhe para o [!UICONTROL Price] e [!UICONTROL Your Price] colunas na [!UICONTROL Plan Details] tabela. Se o feed for descontado:

   * O preço original é marcado com uma linha vermelha.
   * A taxa na variável [!UICONTROL Your Price] será inferior à taxa na variável [!UICONTROL Price] coluna.

No exemplo, o comprador recebe um desconto de 10% na [!UICONTROL Segments and Overlap] plano no **[!UICONTROL Software Audience Feed]**.

![](assets/buyer-discount.png)

## Localizar dados de feed de assinatura {#find-subscribed-data-fee}

Os dados (características) dos feeds de dados aparecem em suas próprias pastas de armazenamento de características. Ir para **[!UICONTROL Audience Data > Traits]** e expanda a variável **[!UICONTROL 3rd-Party Data]** pasta para visualizar e trabalhar com as características nos feeds que você assinou. Procure a subpasta com o nome do seu provedor de dados. Eles contêm pastas chamadas com base no feed de dados individual e nas características de lista fornecidas pelo feed.
...
Os compradores de dados cancelam a inscrição dos feeds de dados e planos no **[!UICONTROL Audience Marketplace > Marketplace]**.
...
1. Clique no nome do feed de dados na [!UICONTROL Marketplace].
1. No [!UICONTROL Use Case] localize o plano que deseja usar e mova a **[!UICONTROL Subscription]** controle deslizante para **[!UICONTROL Off]**.

## Desativação do feed de dados: por que acontece e como responder {#data-feed-deactivation-reasons}

Entrada [!UICONTROL Audience Marketplace], os provedores de dados podem revogar o acesso aos feeds de dados que você assinou. Não se preocupe se isso acontecer com você. Nós temos você coberto. Consulte esta seção para obter os processos e procedimentos relacionados às desativações do feed de dados.
...
* **Faturamento:** Os provedores de dados desativarão um feed se você estiver consistentemente atrasado com o pagamento de taxas ou se não pagar suas taxas.
* **Atualizações do feed:** Os provedores de dados precisam desativar os feeds ao atualizar a taxonomia de feed ou as estruturas de custo.
* **Compradores inativos:** Os provedores de dados se reservam o direito de desativar os feeds se os assinantes não mostrarem gastos durante um período estendido.
* **Vendedores inativos:** Provedores de dados que deixam [!UICONTROL Audience Marketplace] O desativará e excluirá todos os feeds de dados.

>[!TIP]
>
>Entre em contato diretamente com seu provedor de dados se você achar que um feed de dados foi desativado por engano. Seu [!DNL Adobe] o consultor da pode ajudá-lo com informações de contato ou suporte adicional.

## Email de desativação {#deactivation-email}

Quando um provedor de dados desativa um de seus feeds de dados, [!DNL Audience Manager] envia um email para os usuários em sua empresa que [!UICONTROL Administrator] permissões. Às vezes, os filtros de email classificam essa mensagem como spam. Como resultado, você pode perder essa notificação importante. Para ajudar a identificar a mensagem de desativação, esse email contém os seguintes elementos:

* **De:** O email de desativação é proveniente de `aam-noreply@adobe.com`. Dica profissional: não responda a este email.

* **Linha de assunto:** Inscrição em *nome do feed de dados aqui* foi cancelado.

* **Anexos** O e-mail inclui um anexo intitulado &quot; `list-of-affected-entities-by-feed-revocation.csv`.&quot; Essa é uma maneira complicada de dizer que o anexo lista todas as características incluídas no feed cancelado. Como comprador de dados, você deve revisar este anexo. Ele ajudará você a encontrar e remover características desativadas de seus segmentos e [modelos algorítmicos](../../../features/algorithmic-models/understanding-models.md).
...
* Remoção em massa com o [REST APIs](../../../api/rest-api-main/rest-api-main.md) ou o [Ferramentas de gerenciamento em massa](../../../reference/bulk-management-tools/bulk-management-intro.md).

* Pesquise segmentos afetados manualmente e remova características desativadas usando [!UICONTROL Segment Builder]. Consulte [Remover características de um segmento](../../../features/segments/segment-builder.md#segment-builder-controls-traits).
...
[Cancelar inscrição no feed de dados desativado](../../../features/audience-marketplace/marketplace-data-buyers/marketplace-manage-subscriptions.md#unsubscribe) depois de remover todas as características revogadas da conta. Se essa for uma desativação temporária, você poderá assinar novamente depois que o provedor de dados terminar de fazer as alterações necessárias e reativar o feed. Como na maioria dos casos, uma boa comunicação com seus parceiros (o provedor de dados e [!DNL Adobe]) pode ajudá-lo a trabalhar nesse processo.

## Como entender a página de detalhes do plano no Audience Marketplace {#marketplace-buyer-details}

Ao clicar no nome de um plano de dados na variável [!UICONTROL Marketplace], [!DNL Audience Manager] O fornece informações que podem ajudá-lo a fazer escolhas informadas sobre a assinatura de um feed de dados.
...
1. **[!UICONTROL Plan Details]** Tabela. Esta tabela mostra os casos de uso para os quais você pode assinar o feed de dados, bem como seu modelo de preços. Consulte [Noções básicas sobre casos de uso do feed de dados](#use-cases).

1. Botões de ação do plano.
   * Clique em **[!UICONTROL Cancel]** para sair da página sem fazer alterações.
   * Clique em **[!UICONTROL Review & Subscribe]** para assinar um feed de dados. Esse botão fica esmaecido até que você alterne um [!UICONTROL Subscription] alternar para [!UICONTROL On]. Consulte também [Assinar um feed de dados público](#subscript-public-data-feed) e [Assinar um feed de dados privado](#subscript-private-data-feed).

## Noções básicas sobre casos de uso do feed de dados {#use-cases}

Como um [!UICONTROL Audience Marketplace] comprador de dados, você pode comprar dados para sobreposição, modelagem e casos de uso de ativação. Cada caso de uso é projetado para uma finalidade específica e limita o que você pode fazer com os dados. Essas descrições de casos de uso podem ajudá-lo a tomar a decisão certa sobre qual tipo de plano de dados comprar.
...
Esse caso de uso permite comparar suas características com as características do provedor em uma [relatório de sobreposição de característica por característica.](../../../reporting/dynamic-reports/trait-trait-overlap-report.md#trait-to-trait-overlap-report) Além disso, é possível criar ou adicionar características do provedor a um segmento e fazer comparações adicionais com o [segmento por característica](../../../reporting/dynamic-reports/segment-trait-overlap-report.md) e [segmento por segmento](../../../reporting/dynamic-reports/segment-segment-overlap-report.md) relatórios. As comparações de sobreposição podem ajudar a:

* **Estender alcance do público:** A baixa sobreposição sugere que suas características contêm usuários que você não viu antes. Talvez você queira que essas características tentem alcançar novos usuários.
* **Melhorar públicos existentes:** A alta sobreposição sugere que suas características são semelhantes às de propriedade do provedor de dados. Talvez você queira que essas características ajudem a fazer melhorias direcionadas e incrementais em um público já desenvolvido.

### Modelos algorítmicos

Esse caso de uso permite avaliar as características do fornecedor em relação às suas características com o [modelagem algorítmica](../../../features/algorithmic-models/understanding-models.md#understanding-models). Por exemplo, nosso sistema de modelagem algorítmica usa uma de suas características como base de comparação com uma característica do fornecedor. Quando o modelo é executado, ele pode mostrar se os públicos-alvo nas características do fornecedor compartilham atributos de conversão semelhantes às suas características.

### Ativação

Esse caso de uso permite enviar dados para um [destino](../../../features/destinations/destinations.md). Entrada [!DNL Audience Manager], um destino é qualquer sistema de terceiros (servidor de anúncios, [!DNL DSP], [!DNL DMP], exchange etc.) com o qual você deseja compartilhar dados. No entanto, com um [!UICONTROL Activation] caso de uso, não é possível executar relatórios de sobreposição ou testar os dados em um modelo algorítmico.
