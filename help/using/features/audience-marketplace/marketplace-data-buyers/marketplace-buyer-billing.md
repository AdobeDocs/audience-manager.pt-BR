...
keywords: Relatórios de nível de segmento, nível de segmento, nível de segmento
...
[!UICONTROL Audience Marketplace] oferece duas maneiras de relatar o uso [!DNL CPM] :

* **Relatório** em nível de segmento: este é o método de relatório de [!DNL CPM] uso recomendado. Quando você relata o [!DNL CPM] uso no nível do segmento, a seção de relatório no nível do feed de dados é preenchida automaticamente com as quantias de uso correspondentes, com base nos algoritmos descritos em Atribuição [de custo para feeds](#cost-attribution)de dados CPM.
* **Relatório** no nível do feed de dados: esse método exige que você reporte individualmente o [!DNL CPM] uso de cada feed de dados, com base nos algoritmos descritos em Atribuição [de custo para feeds](#cost-attribution)de dados CPM. No entanto, esse método é mais tedioso e propenso a erros do que relatórios de nível de segmento.

<br> 

## Relatório de uso do CPM no nível do segmento {#segment-level-report}
...
Após o [!DNL CPM] uso do relatório no nível do segmento, [!UICONTROL Audience Marketplace] atribui automaticamente o uso correto aos feeds de dados correspondentes, com base na Atribuição [de custo para Feeds](#cost-attribution)de dados do CPM.
...
O relatório de nível de feed de dados é um processo mais tedioso e propenso a erros, já que você deve calcular individualmente o [!DNL CPM] uso de cada feed de dados. Recomendamos que você [reporte o uso do CPM no nível](#segment-level-report) do segmento.
...
Para reduzir erros e sobrecarga ao relatar o [!DNL CPM] uso, você pode usar a opção de relatório em massa para baixar um [!DNL CSV] arquivo que contém os feeds de dados e segmentos, preencher o uso e fazer upload dele de volta para [!DNL Audience Manager]. Você pode usar relatórios em massa para relatar o uso de feed e segmento.
...
| ------------- | -------------| -----|
| Entrada inválida | [!DNL Audience Manager] detectou uma alteração no esquema de [!DNL CSV] arquivos, como colunas ausentes ou alterações nos títulos das colunas. | Evite alterar a estrutura da tabela. |
...
| Registros duplicados encontrados | [!DNL Audience Manager] foram detectados registros duplicados com valores de impressão diferentes. | Revise o relatório e certifique-se de não relatar valores de uso diferentes para o mesmo feed de dados ou segmento. |
...
Em [!UICONTROL Audience Marketplace] você deve relatar valores de impressão a cada mês, para cada um de seus segmentos. Recomendamos o [!DNL CPM] uso de relatórios no nível do segmento, para que a atribuição de custos seja feita automaticamente.
...
>[!TIP]
>Quando você relata o [!DNL CPM] uso no nível do segmento, a seção de relatório no nível do feed de dados é automaticamente preenchida com as quantidades de uso correspondentes.
...
>[!IMPORTANT]
>
>Como comprador, todos os totais de impressões reportados devem ser verdadeiros e exatos. Se você não reportar os totais de impressões no quinto dia de cada mês, deverá incluir os totais do mês não relatado no mês seguinte.
...
>[!TIP]
>[Relatar o uso do CPM no nível](#segment-level-report) do segmento para que os relatórios no nível do feed de dados sejam feitos automaticamente pelo Audience Manager.
...
Esse segmento contém três características de provedores de dados separados. Como a qualificação de segmento se baseia em uma [!UICONTROL OR] condição, os visitantes precisam realizar pelo menos uma das três características para se qualificarem para o segmento.
...
O segmento é mapeado para um destino e 1.000.000 impressões são inseridas para esse segmento em um mês, usando o Relatório [no nível do](#segment-level-report)segmento.
...
Um feed de dados de taxa fixa cobra uma quantia fixa a cada mês, independentemente de quando a assinatura é iniciada ou de quantas impressões você usa. As taxas não são rateadas para uso parcial de mês ou intervalos. Assim como com o faturamento do CPM, a Adobe gerará uma fatura e cobrará uma taxa mensal e fixa para seus feeds de dados inscritos.

Por exemplo, digamos que você decidiu ativar certas características em um feed no meio do mês. Você ainda será cobrado à taxa mensal completa, independentemente de quando tiver iniciado a assinatura ou ativado características específicas.