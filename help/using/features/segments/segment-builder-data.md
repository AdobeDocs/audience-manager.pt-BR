...
Adicionar e remover [!UICONTROL traits] in [!UICONTROL Segment Builder] para ver o real [!UICONTROL trait] junto com os dados de população de segmentos reais e estimados. Os dados de tamanho estimado da população ajudam a criar o segmento certo para a campanha.

## [!UICONTROL Trait] Dados de população {#trait-population-data}

[!UICONTROL Segment Builder] mostra a você [!UICONTROL Total Trait Population] no último dia em que você adicionar um [!UICONTROL trait] para um segmento. Esses dados aparecem no campo azul ao redor do [!UICONTROL trait] no [!UICONTROL Basic View] seção.
...
Entrada [!UICONTROL Segment Builder], uma barra azul nos gráficos de população estimada indica os intervalos possíveis superior e inferior para o tamanho do segmento. Embora o desempenho passado não garanta resultados futuros, os dados estimados podem ajudar a entender o tamanho potencial de um segmento novo ou editado.

## Visão geral dos dados de população do segmento {#segment-populations}

[!UICONTROL Segment Builder] O mostra os dados de preenchimento do segmento à medida que você cria e edita segmentos.

* Para dados estimados da população do segmento (tempo real e total), [!UICONTROL Segment Builder] O não atualiza os gráficos automaticamente à medida que você adiciona ou remove características em um segmento. Clique em **[!UICONTROL Calculate Estimates]** para ver (ou atualizar) os números de população estimados.

* Para dados de população de segmentos reais (tempo real e total), [!UICONTROL Segment Builder] O atualiza o gráfico de segmentos automaticamente quando você carrega um segmento existente. Para novos segmentos, ou quando você adiciona novas características a um segmento existente, os dados da população real não são atualizados até 24 horas após a criação do segmento.
  ...
  <tr> 
   ...
   <td colname="col2"> <p>O número estimado de visitantes únicos vistos em tempo real pelo intervalo de tempo especificado e que foram qualificados para o segmento no momento em que foram vistos pelo Audience Manager. </p> <p>Entrada <span class="wintitle"> Construtor de segmentos</span>, as últimas populações de 30 dias para características (<span class="wintitle"> Total de populações de características</span>), podem ser diferentes para características e segmentos avaliados em tempo real. </p> <p>
     <ul id="ul_CAE803D09913462CAD413A665D85C1A2"> 
      ...
      <li id="li_FF3CACD0B6C742CDB94D66200D77CE06">Para segmentos avaliados em tempo real, a última métrica de 30 dias conta o número de usuários que se qualificaram para uma característica (nesse segmento) em algum ponto no passado e foram vistos novamente por Audience Manager nos últimos 30 dias. Por exemplo, digamos que você tenha um usuário que se qualificou para uma característica de 60 dias atrás e foi visto novamente há 10 dias. Nos dados, esse usuário não será adicionado à contagem de características porque se qualificou pela primeira vez para a característica há mais de 30 dias. No entanto, elas serão incluídas na última contagem de 30 dias para os segmentos avaliados em tempo real. Isso ocorre porque elas se qualificaram para o segmento dentro do intervalo de 30 dias. </li>
     </ul> </p> <p> <p>Observação: a variável <span class="wintitle"> População estimada em tempo real</span> não inclui dispositivos que se qualificaram para um segmento com base nas conexões fornecidas por um <span class="wintitle"> Regra de mesclagem de perfis</span> que usa um <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"> opção de gráfico de dispositivo</a>. </p> </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>O número estimado de visitantes únicos que podem estar em seu segmento novo ou modificado. Como em quase qualquer estimativa, o desempenho passado não garante resultados futuros, mas você pode usar o total estimado para: </p> <p> 
     ...
     </ul> </p> <p> <p>Observação: a variável <span class="wintitle"> População Total Estimada</span> não inclui dispositivos que se qualificaram para um segmento com base nas conexões fornecidas por um <span class="wintitle"> Regra de mesclagem de perfis</span> que usa um <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"> opção de gráfico de dispositivo</a>. </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
[!UICONTROL Profile Merge Rules] afetam os números reais e da população total. Esses totais variam dependendo se a variável [!UICONTROL Profile Merge Rule] um segmento pertencente ao usa ou não uma opção de gráfico de dispositivos. Consulte também, [Definição das Opções de Regra de Mesclagem de Perfis](../../features/profile-merge-rules/merge-rule-definitions.md).

### Dados de população do segmento para [!UICONTROL Merge Rules] Sem um [!UICONTROL Device Graph Option]

A tabela a seguir define as métricas de população total e em tempo real quando seus segmentos são usados por um [!UICONTROL Profile Merge Rule] criado sem um [!UICONTROL device graph] opção. Estas são as configurações de opções do dispositivo **[!UICONTROL No Device Options]** e **[!UICONTROL Current Device Proflie]**.
...
### Dados de população do segmento para [!UICONTROL Merge Rules] Com um [!UICONTROL Device Graph] Opção

A tabela a seguir define as métricas de população total e em tempo real quando seus segmentos são usados por um [!UICONTROL Profile Merge Rule] criado com um [!DNL device graph] opção. Estas são as configurações das opções de dispositivo para o [!UICONTROL Profile Link Device Graph], o [!DNL Adobe] [!DNL device graph], e outros [!DNL device graph] opções disponíveis para você.
...
[!UICONTROL Segment Builder] O oferece suporte a estimativas de tamanho de segmento para regras de segmento que contêm até quatro expressões de recenticidade e frequência. Escolher mais de quatro expressões de recenticidade e frequência ao criar uma regra de segmento faz com que o avaliador de segmento mostre um erro ao estimar a população.

### Limitações devidas a [!UICONTROL Merge Rules] Ao Estimar Populações De Segmentos

Atualmente, há uma limitação conhecida porque nosso avaliador de tamanho de segmento não leva em conta [!UICONTROL profile merge rules]. Por exemplo, verifique segmentos com a variável **[!UICONTROL No Authenticated Profile + Current Device Profile]** [regra de mesclagem](../../features/profile-merge-rules/merge-rule-definitions.md). Devido à maneira como calculamos os números de estimativa do segmento atualmente, as populações estimadas incluirão perfis autenticados. No entanto, as populações de segmentos existentes ignorarão corretamente os perfis autenticados.
