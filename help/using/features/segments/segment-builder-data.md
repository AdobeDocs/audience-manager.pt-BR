...
Adicione e remova [!UICONTROL traits] em [!UICONTROL Segment Builder] para ver as populações [!UICONTROL trait] reais, juntamente com os dados reais e estimados da população do segmento. Os dados estimados do tamanho da população ajudam a criar o segmento certo para sua campanha.

## [!UICONTROL Trait] Dados de preenchimento  {#trait-population-data}

[!UICONTROL Segment Builder] mostra  [!UICONTROL Total Trait Population] o último dia quando você adiciona um item  [!UICONTROL trait] a um segmento. Esses dados aparecem no campo azul ao redor de [!UICONTROL trait] selecionado na seção [!UICONTROL Basic View].
...
## Calculando preenchimento de segmentos reais e estimados {#calculating-real-estimated-populations}

Quando você cria um novo segmento ou altera um segmento existente, o Audience Manager demora até 24 horas para exibir os resultados das populações de segmentos em tempo real e total.
...
* Para dados estimados de população de segmentos (tempo real e total), [!UICONTROL Segment Builder] não atualiza os gráficos automaticamente à medida que você adiciona ou remove características em um segmento. Clique em **[!UICONTROL Calculate Estimates]** para ver (ou atualizar) os números de população estimados.
...
## Dados de preenchimento de segmento estimado {#estimated-segment-population} Definidos
...
<table id="table_B24503F372E34B6BBDF5204181701A59"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>O número estimado de visitantes únicos vistos em tempo real para o intervalo de tempo especificado e que eram qualificados para o segmento no momento em que eram vistos pelo Audience Manager. </p> <p>Em <span class="wintitle"> Construtor de segmentos</span>, os últimos 30 dias de populações de características (<span class="wintitle"> Total de preenchimento de características</span>) podem ser diferentes para características e segmentos avaliados em tempo real. </p> <p>
     ...
     </ul> </p> <p> <p>Observação: A métrica <span class="wintitle"> Preenchimento em tempo real estimado</span> não inclui dispositivos que se qualificaram para um segmento com base nas conexões fornecidas por uma <span class="wintitle"> Regra de mesclagem de Perfil</span> que usa uma opção de gráfico de dispositivo <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"></a>. </p> </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>O número estimado de visitantes únicos que podem estar em seu segmento novo ou modificado. Como em quase qualquer estimativa, o desempenho passado não garante resultados futuros, mas você pode usar o total estimado para: </p> <p> 
     ...
     </ul> </p> <p> <p>Observação: A métrica <span class="wintitle"> Preenchimento total estimado</span> não inclui dispositivos que se qualificaram para um segmento com base nas conexões fornecidas por uma <span class="wintitle"> Regra de mesclagem de Perfil</span> que usa uma opção de gráfico de dispositivo <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"></a>. </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
[!UICONTROL Profile Merge Rules] afetam os números efetivos em tempo real e totais da população. Esses totais variam dependendo se [!UICONTROL Profile Merge Rule] um segmento pertence ou não a uma opção de gráfico de dispositivo. Consulte também, [Opções de Regra de Mesclagem de Perfis Definidas](../../features/profile-merge-rules/merge-rule-definitions.md).

### Dados de preenchimento de segmento para [!UICONTROL Merge Rules] sem um [!UICONTROL Device Graph Option]

A tabela a seguir define as métricas reais em tempo real e total de população quando seus segmentos são usados por [!UICONTROL Profile Merge Rule] criados sem uma opção [!UICONTROL device graph]. Estas são as configurações de opções do dispositivo **[!UICONTROL No Device Options]** e **[!UICONTROL Current Device Proflie]**.
...
### Dados de preenchimento de segmento para [!UICONTROL Merge Rules] com uma opção [!UICONTROL Device Graph]

A tabela a seguir define as métricas reais em tempo real e total de população quando seus segmentos são usados por [!UICONTROL Profile Merge Rule] criados com uma opção [!DNL device graph]. Estas são as configurações de opções do dispositivo para as [!UICONTROL Profile Link Device Graph], [!DNL Adobe] [!DNL device graph] e outras [!DNL device graph] opções de terceiros que estão disponíveis para você.
...
Atualmente, há uma limitação conhecida, pois nosso avaliador de tamanho de segmento não é responsável por [!UICONTROL profile merge rules]. Por exemplo, observe os segmentos com a **[!UICONTROL No Authenticated Profile + Current Device Profile]** [regra de mesclagem](../../features/profile-merge-rules/merge-rule-definitions.md). Devido à forma como atualmente calculamos os números de estimativa de segmento, as populações estimadas incluirão perfis autenticados. No entanto, as populações de segmentos existentes ignorarão corretamente os perfis autenticados.
