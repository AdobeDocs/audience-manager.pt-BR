---
title: Migração da extensão de tag do Audience Manager para a extensão de tag da Web SDK
description: Entenda as etapas para atualizar sua biblioteca de coleção de dados para o Audience Manager da extensão de tag da Audience Manager para a extensão de tag da Web SDK
...
# Atualize sua biblioteca de coleta de dados para o Audience Manager da extensão de tag da Audience Manager para a extensão de tag da Web SDK
...
Siga as instruções abaixo para criar um fluxo de dados na Coleção de dados da Adobe Experience Platform. Ao enviar dados para esse fluxo de dados, ele encaminha dados para a Audience Manager. No futuro, esse mesmo fluxo de dados encaminha dados para o Real-Time CDP.
...
Seu fluxo de dados agora está pronto para receber e transmitir dados para a Audience Manager.
...
+++**4. Atualizar regras para usar a extensão Web SDK em vez da extensão Audience Manager**

Essa etapa contém a maior parte do esforço necessário para migrar para o Web SDK e requer conhecimento de como a implementação funciona. Um exemplo é fornecido abaixo de como editar uma regra de tag típica. Atualize todas as regras de tag na implementação para substituir todas as referências à extensão do Audience Manager pela extensão do Web SDK.
...
1. Observe todas as variáveis do Audience Manager definidas nessa regra. Inclua as variáveis definidas nos menus suspensos e as variáveis definidas no código personalizado.
...
1. Defina os pares de valor-chave do Audience Manager com os mesmos valores que foram configurados na extensão do Audience Manager.
1. Depois que toda a lógica da regra for replicada usando a extensão Web SDK, selecione **[!UICONTROL Keep Changes]**.
1. Repita essas etapas para cada configuração de ação que usa a extensão de tag do Audience Manager para definir valores.
...
1. Selecione uma regra que envia um evento Web SDK.
...
1. Repita essas etapas para cada configuração de ação que usa o Audience Manager para enviar um evento.
...
+++**6. Desabilitar extensão do Audience Manager**

Depois que a implementação de tag for totalmente migrada para o Web SDK, você poderá desativar a extensão do Audience Manager.
...
Neste ponto, a implementação do Audience Manager é totalmente migrada para o Web SDK e está preparada para mudar para o Real-Time CDP no futuro.
