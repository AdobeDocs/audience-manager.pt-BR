...
Este documento fornece uma visão geral do gerenciamento de privacidade de dados individuais e de solicitações de recusa que você pode enviar para o [!DNL Audience Manager] por meio da [IU DO PRIVACY SERVICE](https://privacyui.cloud.adobe.io/) e a variável **[!DNL Privacy Service API]**.

Essas ferramentas permitem enviar solicitações de privacidade de dados do consumidor feitas no [!DNL GDPR] e [!DNL CCPA].
...
É possível enviar solicitações individuais para acessar e excluir dados do consumidor do [!DNL Audience Manager], de duas formas:
...
Ao enviar solicitações individuais de privacidade de dados, você pode enviar qualquer [!DNL Audience Manager] identificadores (IDs), conforme descrito na seção **[Identificadores Audience Manager](data-privacy-ids.md)** juntamente com suas respectivas IDs de namespace (IDs de fonte de dados).
...
É possível enviar solicitações de exclusão de dados por meio da [IU DO PRIVACY SERVICE](https://privacyui.cloud.adobe.io) (documentação) [aqui](https://experienceleague.adobe.com/docs/experience-platform/privacy/ui/overview.html?lang=en) ou chamando a API do Privacy Service (documentação [aqui](https://experienceleague.adobe.com/docs/experience-platform/privacy/api/overview.html?lang=en) e [!DNL API] referência [aqui](https://developer.adobe.com/experience-platform-apis/references/privacy-service/).

A [Interface do usuário do Privacy Service](https://privacyui.cloud.adobe.io/) permite criar novas solicitações de trabalho usando o [!UICONTROL Request Builder] ou fazendo upload de um arquivo [!DNL JSON].

Para ver a aparência de um arquivo válido [!DNL JSON], é possível [ baixar uma amostra de JSON](../data-security-and-privacy/assets/access_request.json).

A Adobe entende seu compromisso de atender às solicitações de privacidade dos clientes em 30 dias. Por esse motivo, [!DNL Adobe] O está comprometido a processar sua solicitação de exclusão de dados o mais rápido possível.

Em resposta às solicitações de exclusão de dados do consumidor, [!DNL Audience Manager] exclui características e segmentos associados à [!DNL Audience Manager] identificador incluído na solicitação. Além disso, as respectivas [!DNL Audience Manager] identificadores para o indivíduo que recusou a recolha de dados [!DNL Audience Manager] e os respectivos mapeamentos de ID serão removidos.
...
1. Não é possível dar suporte a solicitações de cancelamento de segmentos (ou remover segmentos) do [!DNL Audience Manager] e/ou
2. Não podem receber atualizações do [!DNL Audience Manager] com uma frequência inferior a 30 dias. Nesses casos, [!DNL Audience Manager] os clientes não conseguem enviar solicitações de exclusão para parceiros de ativação de forma automatizada pelo [!DNL Audience Manager].
...
[!DNL Audience Manager] O oferece suporte a padrões do setor em relação ao gerenciamento de recusa. Leia para obter informações completas sobre os tipos de recusa compatíveis com o [!DNL Audience Manager].

Enquanto as solicitações de acesso e exclusão de dados são tratadas pelo [Privacy Service](https://experienceleague.adobe.com/docs/experience-platform/privacy/home.html?lang=en), as solicitações de recusa são feitas por meio da [!DNL DCS API]. Leia para saber o que foi recusado [!DNL API] As chamadas do devem parecer com.

### Solicitações globais de recusa

A recusa global representa uma opção de não participação em [!DNL Audience Manager] e outros [!DNL Adobe Experience Cloud] para todas as marcas. A tabela abaixo lista os métodos usados para a recusa global:
...
A recusa em nível de parceiro permite excluir usuários da coleta de dados por [!DNL Audience Manager] parceiros. Você pode enviar solicitações de recusa em nível de parceiro para IDs de vários dispositivos, incluindo [!DNL CRM] IDs e endereços de email com hash.
...
* [!DNL Audience Manager] cancela a segmentação da opção de não participação [!DNL CRM] ID e última ID de dispositivo de todos os segmentos;
* [!UICONTROL Destination] os parceiros recebem a solicitação de cancelamento de segmentos para o [!DNL CRM] ID e ID do último dispositivo. O cancelamento de segmentos funciona para destinos [em tempo real](data-privacy-requests.md#aam-partners-with-unsegmentation) e em lote.
* Nenhum dado histórico é excluído.

Quando [!DNL Audience Manager] recebe uma solicitação de recusa em nível de parceiro, a variável [!DNL JSON] retornado pelo [!DNL DCS] contém o [código de erro 171](../../api/dcs-intro/dcs-api-reference/dcs-error-codes.md#opt-out-error-codes), com a mensagem [!UICONTROL "Encountered opt out tag"], em vez de [!DNL Audience Manager] ID de usuário.
...
| Múltiplo  `d_cid`  e  `d_cid_ic`  pares de valor-chave. | `https://yourcompany.demdex.net/demoptout?d_cid=123%01987&d_cid_ic=456%01321...` |

 

### Recusa em nível de parceiro com chamadas de ID de dispositivo

A recusa em nível de parceiro permite excluir usuários da coleta de dados por [!DNL Audience Manager] parceiros. Você pode recusar a coleta de dados em uma determinada ID de dispositivo para uma marca fazendo as seguintes chamadas para a [DCS API](../../api/dcs-intro/dcs-api-reference/dcs-api-reference-overview.md):
...
Para ajudar você a automatizar as solicitações de privacidade de dados do consumidor, [!DNL Audience Manager] O tentará notificar os parceiros de ativação sobre solicitações de exclusão dos titulares de dados, enviando a eles informações de cancelamento de segmentos (ou removendo segmentos).
...
Tendo em conta [!DNL Audience Manager] não é a fonte dos dados, há um papel limitado para a correção de dados no [!DNL Audience Manager]. A correção poderia significar que o consumidor pediu para ser desqualificado de um erro [!UICONTROL trait]/[!UICONTROL segment] ou qualificado para o desejado [!UICONTROL trait]/[!UICONTROL segment].

[!DNL Audience Manager] os clientes podem optar por capturar os sinais/características/segmentos relevantes em relação aos perfis do usuário e enviar essas informações por meio do [assimilação de dados offline](../../integration/sending-audience-data/batch-data-transfer-explained/batch-data-transfer-overview.md) para [!DNL Audience Manager]. Observe que o usuário continuará se qualificando para o original [!UICONTROL trait] e [!UICONTROL segments] se eles repetirem seu comportamento.
