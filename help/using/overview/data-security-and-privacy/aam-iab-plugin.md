...
>[!IMPORTANT]
>
>Audience Manager está registrado no [TCF do IAB](https://iabeurope.eu/tcf-for-vendors/) com a ID de fornecedor 565.
...
Clientes que estão atualizando seus [!DNL Audience Manager Plug-in for IAB TCF] implementação a partir de [!DNL IAB TCF] v1.1 para [!DNL IAB TCF] v2.0, ou ativar [!DNL IAB TCF] v2.0 pela primeira vez, todas devem seguir as mesmas diretrizes em pré-requisitos e implementação, conforme descrito abaixo.
...
>
> Os clientes que desejam continuar usando o plug-in do Audience Manager para TCF do IAB para gerenciamento de consentimento devem atualizar para a versão mais recente de [ECID](https://github.com/Adobe-Marketing-Cloud/id-service/releases) para apoio contínuo.
>
> Depois de atualizar para o mais recente [ECID](https://github.com/Adobe-Marketing-Cloud/id-service/releases) A versão, as sequências de consentimento da TCF do IAB v1.1 não serão mais compatíveis, portanto, atualize sua CMP antes de atualizar para a versão mais recente da ECID.
...
2. Você deve usar o Audience Manager [!DNL Data Integration Library] (DIL) versão 9.0 ou mais recente, disponível para download em [here](https://github.com/Adobe-Marketing-Cloud/dil/releases). Leia sobre a [DIL na documentação do Audience Manager](../../dil/dil-overview.md). Recomendamos usar o [Extensão de tag do Adobe Audience Manager](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/audience-manager/overview.html) para a implantação mais fácil do DIL do Audience Manager.
3. Como alternativa, se você usar [!DNL Server-Side Forwarding] (SSF) para importar dados para o Audience Manager, é necessário atualizar para a versão mais recente do AppMeasurement. Baixe o AppMeasurement usando o [Gerenciador de código do Analytics](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/code-manager-admin.html).
...
>[!WARNING]
>
>Se estiver usando uma Plataforma de gerenciamento de consentimento (CMP) que não é compatível com a TCF do IAB v.2.0, o Audience Manager enviará automaticamente a variável `gdpr=0` em sincronizações de ID, mesmo se os visitantes estiverem na União Europeia. Para determinar se a validação do GDPR está ativa, recomendamos que você confirme com sua Plataforma de gerenciamento de consentimento (CMP) que ela suporta a TCF do IAB v2.0.
...
A maneira mais fácil de fazer isso é usando [Tags do Adobe Experience Platform](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=en) para adicionar [!DNL ECID Opt-in] nas propriedades. Leia a documentação do [Extensão de Opt-in da ECID](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html) para saber como configurar a extensão Tags.
...
Os usuários informam suas opções na forma de *consentimento* e *interesse legítimo* para efeitos do IAB *fornecedores terceirizados* registrado na lista global de fornecedores.
...
Os detalhes sobre as várias finalidades e permissões incluídas na TCF do IAB v2.0 são abordados na seção [Políticas de Estrutura de Transparência e Consentimento do IAB Europe](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes).
...
O Audience Manager avalia as opções dos usuários armazenadas na cadeia de caracteres TC IAB para os seguintes fins, definidos na variável [Políticas de Estrutura de Transparência e Consentimento do IAB Europe](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#Appendix_A_Purposes_and_Features_Definitions).
...
>
>Por [Regulamentos IAB](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#Special_Purpose_1__Ensure_security_prevent_fraud_and_debug_), Finalidade especial 1 (garantir a segurança, evitar fraudes e depurar) é sempre consentido e os usuários não podem se opor a ele.
...
4. Se o GDPR for aplicado, o Audience Manager verifica a string TC IAB, transmitida no `gdpr_consent` , para o consentimento necessário. O Audience Manager precisa de consentimento para armazenar e/ou acessar informações em um dispositivo ([TCF do IAB finalidade 1](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes), desenvolvimento e melhoria dos produtos ([TCF do IAB finalidade 10](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes)), além do consentimento do fornecedor do Audience Manager para armazenar, processar ou ativar dados.
5. Se a cadeia de caracteres TC IAB estiver presente e contiver o consentimento necessário, o Audience Manager transmitirá a cadeia de caracteres TC IAB para nossa [servidores de coleta de dados](../../reference/system-components/components-data-collection.md) (DCS).
6. O Audience Manager responde definindo um [cookie demdex](https://experienceleague.adobe.com/docs/core-services/interface/ec-cookies/cookies-am.html) no navegador, e inicia e respeita sincronizações de IDs de terceiros.
...
>[!IMPORTANT]
>
>Se você estiver trabalhando com parceiros de destino do Audience Manager que exigem parâmetros da TCF do IAB, mas não tem uma CMP compatível com a TCF do IAB no seu site, o Audience Manager envia `gdpr=0` em sincronizações de ID. Isso significa que o GDPR não se aplica a esses usuários.
>
...
1. O usuário recebe uma impressão por meio de um servidor de anúncios. Isso se traduz em um [chamada de pixel](../../integration/media-data-integration/impression-data-pixels.md) para nossos servidores de coleta de dados (DCS).
2. O Audience Manager verifica se o sinalizador de GDPR se aplica. Caso contrário, o Audience Manager armazenará os dados transmitidos no `gdpr` e `gdpr_consent` em chamadas de pixel.
3. Se a string IAB TC estiver presente e tiver as permissões necessárias, o Audience Manager armazenará os dados transmitidos no `gdpr` e `gdpr_consent` em chamadas de pixel.
4. Se a cadeia de caracteres IAB TC estiver ausente ou não tiver as permissões necessárias, o Audience Manager descartará os dados transmitidos no `gdpr` e `gdpr_consent` em chamadas de pixel.
...
A integração do Audience Manager com a TCF do IAB v2.0 é compatível com o anexo do consentimento para as informações enviadas para o [Destinos de URL](../../features/destinations/create-url-destination.md) que são integrados ao IAB TCF v2.0. No entanto, esse processo não é feito automaticamente pelo Audience Manager para evitar a quebra de formatos específicos de URL.

Clientes que desejam anexar o consentimento aos dados enviados para o [!DNL URL destinations] deve adicionar manualmente o `${GDPR}` e `${GDPR_CONSENT_XXXX}` macros para seu formato de URL, substituindo `XXXX` com a ID do parceiro de destino.
...
O Plug-in do Audience Manager para a TCF do IAB recusa automaticamente as IDs presentes em uma solicitação, quando os visitantes do site não fornecem as permissões apropriadas. Se a solicitação contiver um [ID entre dispositivos (CRM ID)](../../reference/ids-in-aam.md), o Audience Manager recusa a ID, juntamente com o último dispositivo vinculado a ela [ID entre dispositivos (CRM ID)](../../reference/ids-in-aam.md).

## Testar a implementação do IAB {#test-iab-implementation}

Para testar se você implementou corretamente o Plug-in do Audience Manager para TCF do IAB, leia [Caso de uso 4 na validação do serviço de Opt-in](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/testing-optin-and-iab-plugin.html#section-64331998954d4892960dcecd744a6d88).
