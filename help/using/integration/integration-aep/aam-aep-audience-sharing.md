...
Você precisa do [[!DNL Audience Manager source connector]](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/audience-manager.html) e a variável [Públicos do Experience Cloud](https://experienceleague.adobe.com/docs/experience-platform/destinations/catalog/adobe/experience-cloud-audiences.html) destino no Experience Platform para permitir o compartilhamento de público entre o Audience Manager e o Adobe Experience Platform.

Você pode usar características e segmentos de Audience Manager no Experience Platform para adicionar dados de Audience Manager aos perfis de clientes e se beneficiar do Experience Platform [serviço de segmentação](https://experienceleague.adobe.com/docs/experience-platform/segmentation/home.html?lang=en).
...
* Ative seus segmentos para destinos que ainda não são compatíveis com o Experience Platform [catálogo de destinos](https://experienceleague.adobe.com/docs/experience-platform/rtcdp/destinations/destinations-cat/destinations-catalog.html).

Além disso, seus segmentos de Experience Platform são compartilhados para outras soluções de Experience Cloud, via [Serviços principais](https://experienceleague.adobe.com/docs/core-services/interface/experience-cloud.html).
...
|---------|----------|---------|---------|
| **Compartilhamento de público** | <ul><li>Enriqueça os perfis do cliente com dados de Audience Manager</li><li>Usar dados de Audience Manager na segmentação de Experience Platform</li></ul> | <ul><li>Adicionar dados de terceiros a segmentos</li><li>Modelagem algoritmica</li><li>Ativação para destinos adicionais</li></ul> | Use segmentos de Experience Platform em outras soluções de Experience Cloud, como Adobe Target, Advertising Cloud ou Marketo. |
...
* [origem do Audience Manager](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/audience-manager.html)
* [Criar uma conexão de origem do Adobe Audience Manager na interface](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/adobe-applications/audience-manager.html?lang=en)

>[!IMPORTANT]
>
>O Adobe incentiva os clientes a configurar a conexão sem selecionar o **[!UICONTROL Select all segments]** e **[!UICONTROL Select all traits]** conforme mostrado abaixo. A assimilação de populações consideráveis de segmentos de Audience Manager tem um impacto direto na contagem total de perfis ao enviar pela primeira vez um segmento de Audience Manager para a Platform usando a origem de Audience Manager. Isso significa que selecionar todos os segmentos pode levar a uma contagem de perfis que excede seu direito de uso de licença.
>
>![Captura de tela que mostra as opções Selecionar todos os segmentos e Selecionar todas as características desmarcadas no fluxo de trabalho para se conectar ao conector de origem do Audience Manager.](/help/using/integration/integration-aep/assets/select-all-segments-traits-unchecked.png)

### Usar características e segmentos de Audience Manager no Experience Platform {#use-aam-data-in-aep}

Após configurar o conector de origem do Audience Manager para importar características e segmentos do Audience Manager, seus dados de Audience Manager aparecem no Experience Platform como **Públicos-alvo** no workflow do segmento. Para obter mais informações sobre os segmentos de Audience Manager e características no Experience Platform, leia:
...
>[!IMPORTANT]
>
> Esta seção descreve a integração de compartilhamento de segmento herdado do Experience Platform para o Audience Manager. Agora é possível configurar essa integração sem o suporte dos representantes do cliente do Adobe. Para obter mais informações, leia a [Públicos do Experience Cloud](https://experienceleague.adobe.com/docs/experience-platform/destinations/catalog/adobe/experience-cloud-audiences.html) documentação de destino.
...
Selecionar **[!UICONTROL Audience Data > Signals > General Online Data]** e pesquisar por `SegId` para localizar sinais provenientes do Experience Platform. Você pode usar essa tela para fins de depuração, para verificar se a integração entre o Experience Platform e o Audience Manager foi configurada corretamente.
...
O Audience Manager cria automaticamente uma pasta de características chamada **Características de Experience Platform** no armazenamento de características.
...
O Audience Manager cria automaticamente uma pasta de segmento chamada **Segmentos Experience Platform** no armazenamento do seu segmento.
...
Para impor a conformidade com o uso de dados no Experience Platform, todos os conjuntos de dados e campos aplicáveis devem ser fornecidos de forma apropriada [rótulos de uso de dados](https://experienceleague.adobe.com/docs/experience-platform/data-governance/labels/overview.html). Além disso, [políticas de uso de dados](https://experienceleague.adobe.com/docs/experience-platform/data-governance/policies/overview.html?lang=pt-BR) devem ser ativadas para ações de marketing específicas contra esses rótulos, conforme descrito pelo [Estrutura DULE (Label Usage Labeling and Enforcement)](https://experienceleague.adobe.com/docs/experience-platform/data-governance/home.html#dule-framework).
...
>[!NOTE]
>
>Para obter informações mais gerais sobre Controles da exportação de dados, consulte o [Documentação dos Controles da exportação de dados](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/data-export-controls.html).
>
...
| Esse destino pode ser usado para direcionamento de anúncios no local | Publicidade no local |
| Esse destino pode ser usado para personalização de anúncios no local | Personalização no local |
...
>[!NOTE]
>
> Ao compartilhar segmentos do Experience Platform para o Audience Manager, a organização da sua plataforma [política de mesclagem padrão](https://experienceleague.adobe.com/docs/experience-platform/profile/merge-policies/overview.html?lang=en#default-merge-policy) tem precedência sobre o [política de mesclagem usada pelo segmento](https://experienceleague.adobe.com/docs/experience-platform/segmentation/ui/segment-builder.html?lang=en#merge-policies) compartilhado com o Audience Manager. Por exemplo, se a política de mesclagem do segmento compartilhado permitir [Identificação de ID](https://experienceleague.adobe.com/docs/experience-platform/profile/merge-policies/ui-guide.html?lang=en#configure), mas a política de mesclagem padrão da organização não, isso pode resultar em diferenças de população entre a Platform e o Audience Manager.

### Composição do segmento no Experience Platform

A integração entre o Adobe Experience Platform e o Audience Manager compartilha uma série de [namespaces de identidade](https://experienceleague.adobe.com/docs/experience-platform/identity/namespaces.html#identity-types) para todos os clientes: ECID, IDFA, GAID, endereços de email com hash (EMAIL_LC_SHA256), AdCloud ID. Se os segmentos de Experience Platform usarem qualquer um desses itens como identidade principal para os perfis qualificados, os perfis serão contados em características e segmentos de Audience Manager.
...
![compartilhamento de segmento Experience Platform para Audience Manager - composição de segmento](assets/AEP-to-AAM-profiles.png)
