...
| [!DNL ECID], [!DNL MID] | Experience Cloud ID. A Experience Cloud ID ([!DNL ECID], abreviações herdadas [!DNL MID] ou [!DNL MCID]) é derivada matematicamente da ID da empresa e da ID de usuário exclusiva do Audience Manager. As long as these IDs remain constant, generating the right [!DNL ECID] for a specific user is simply a math problem. With the same organization ID and Audience Manager [!DNL UUID] you get the same [!DNL ECID] value every time. Você pode ler mais sobre a ECID na documentação [Cookies e Experience Cloud ID](https://docs.adobe.com/content/help/en/id-service/using/intro/cookies.html) . | `mid = 08382830887934830189014177072406221371` | Não visível na interface do usuário do Audience Manager. |
| [!DNL SID] | ID da característica. A ID de característica identifica exclusivamente características no ambiente do Audience Manager. | Em [!DNL DCS] chamadas, `SID` é precedido pelo `d_` prefixo. <p>Exemplo `d_sid=289983`</p> | Uma ID de característica é atribuída a cada característica e é visível na interface do usuário, na [!UICONTROL Traits] página. |
| [!DNL SID] | ID do segmento. A ID do segmento identifica exclusivamente segmentos no ambiente do Audience Manager. | Em [!DNL DCS] chamadas, `SID` é precedido pelo `d_` prefixo. <p>Exemplo `d_sid=4798574` | Uma ID de segmento é atribuída a cada segmento e é visível na interface do usuário, na [!UICONTROL Segments] página. |
| [!DNL csegID] | ID do segmento herdado. Essa ID identifica exclusivamente segmentos no ambiente do Audience Manager. | `741232` | Uma ID de segmento herdada é atribuída a cada segmento e visível na interface do usuário, na [!UICONTROL Segments] página. |
| [!DNL destID] | ID de destino. A ID de destino identifica exclusivamente os destinos no ambiente do Audience Manager. Uma ID é atribuída a cada destino na interface do usuário. | `2523` | Uma ID de destino é atribuída a cada destino e é visível na interface do usuário, na [!DNL Destinations] página. |
| [!DNL DPID] | ID da fonte de dados (também chamada de ID do provedor de dados). A ID da fonte de dados é um namespace para IDs ou características. Uma ID é atribuída a cada fonte de dados (provedor de dados) na interface. | Em [!DNL DCS] chamadas, `dpid` é precedido pelo `d_` prefixo. <p>Exemplo: `d_dpid=39217` | Uma ID do provedor de dados é atribuída a cada fonte de dados e é visível na interface do usuário, na [!UICONTROL Data Sources] página. |
| [!DNL DPUUID] | ID de usuário único do provedor de dados (também chamada de [!DNL CRM ID]). Uma ID de terceiros. Essa é a ID pela qual você identifica usuários finais em seu próprio [!DNL CRM] sistema. Você pode sincronizar [!DNL DPUUIDs] com o Audience Manager [!DNL UUIDs] e pode sincronizar [!DNL DPUUIDs] a partir de diferentes Fontes de Dados ([!DNL DPIDs]) no processo de sincronização de ID. | Nas chamadas do DCS, `dpuuid` é precedido pelo `d_` prefixo. <p> Exemplo `d_dpuuid=2132-3423vn-343fds-3432r` </p> | Não visível na interface do usuário do Audience Manager. |
| [!DNL CRM ID] | Consulte `DPUUID`. | Consulte `DPUUID`. | Consulte `DPUUID`. |
| [!DNL CID], [!DNL CID_IC] | ID do cliente, Código de integração da ID do cliente. Os pares [!DNL CID] e de [!DNL CID_IC] valor chave substituem [!DNL DPID] e [!DNL DPUUID]. Eles fornecem as mesmas funções que o [!DNL DPID] e [!DNL DPUUID], mas são mais eficientes porque incluem a ID do provedor de dados e a ID do usuário (ou código de integração) em um único par de valores chave. | Em chamadas DCS, essas IDs são precedidas pelo `d_` prefixo. <p>Exemplo: `d_cid_ic=39217_myIntegrationCode`</p> | Consulte `DPID` e `DPUUID`. |
