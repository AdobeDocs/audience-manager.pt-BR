...
| Tópico | Descrição |
|----|----|
| [Públicos preditivos](/help/using/features/algorithmic-models/predictive-audiences.md) | <ul><li>**Clonagem de modelo de Predictive Audiences**: Agora você pode clonar modelos existentes e alterar a configuração de acordo com as suas necessidades. Ler [Clonagem e edição de modelos de públicos preditivos](/help/using/features/algorithmic-models/predictive-audiences.md#clone-predictive-audiences) para obter mais informações.</li><li>**Classificação em lote de Predictive Audiences**: além de classificar visitantes em personas distintas em tempo real, o Predictive Audiences agora oferece suporte à classificação em lote, ajudando a classificar usuários integrados e a ativar para destinos em lote.</li></ul> |
| [Relatórios de sobreposição](/help/using/reporting/dynamic-reports/dynamic-reports.md) | Atualizamos a metodologia que usamos para calcular relatórios de sobreposição ([Característica por característica](/help/using/reporting/dynamic-reports/trait-trait-overlap-report.md), [Segmento para característica](/help/using/reporting/dynamic-reports/segment-trait-overlap-report.md), [Segmento por segmento](/help/using/reporting/dynamic-reports/segment-segment-overlap-report.md)). O cálculo de relatórios de sobreposição agora se baseia em únicos de relatórios e assinaturas MinHash (em vez de [Amostragem de dados 1/54](/help/using/reporting/report-sampling.md#data-sampling-ratio)). |
...
| [Nomes de arquivo S3 de entrada](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) e [nomes de arquivo FTP de entrada](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | Atualizamos as definições e fornecemos exemplos para a `DPID` e `DPID_TARGET_DATA_OWNER` campos na sintaxe do nome do arquivo. |

## Agosto de 2020 {#aam-august-updates}

| Tópico | Descrição |
|---- |----|
| [Correspondência de cliente do Google para destinos com base em pessoas](../features/destinations/people-based-destinations-prerequisites.md#gcm) | Agora você pode usar [!UICONTROL People-Based Destinations] para direcionar usuários por meio de [!DNL Google Customer Match]. Antes que você possa usar [!UICONTROL People-Based Destinations] para enviar seus segmentos de público-alvo primários para um [!DNL Google Customer Match] destino, é obrigatório que [!DNL Google] O adiciona você à lista de permissões deles. Verifique a [Destinos com base em pessoas](../features/destinations/people-based-destinations-prerequisites.md#gcm) para obter mais detalhes. |
| [Novos servidores SFTP](../faq/faq-data-collection.md) | Agora você pode usar o novo [!DNL SFTP] servidores ([!DNL ftp-in-gtw] e [!DNL ftp-out-gtw]) que usam [!DNL AWS SFTP Gateway] sobre [!DNL S3]. |
...
| [Introdução às REST APIs do Audience Manager](/help/using/api/rest-api-main/aam-api-getting-started.md#jwt) | Adicionamos uma nova seção, **Autenticação JWT (conta de serviço) usando[!DNL Adobe I/O]**. Consulte esta seção para autenticar nas APIs do Audience Manager usando [!DNL Adobe I/O]. |
