...
| Tópico | Descrição |
|----|----|
| [Públicos preditivos](/help/using/features/algorithmic-models/predictive-audiences.md) | <ul><li>**Clonagem** do modelo de Audiências preditivas: Agora você pode clonar modelos existentes e alterar sua configuração com base em suas necessidades. Leia [Clonando e editando modelos de Audiência preditiva](/help/using/features/algorithmic-models/predictive-audiences.md#clone-predictive-audiences) para obter mais informações.</li><li>**Classificação** em lote de Audiências preditivas: Além de classificar visitantes em personas distintas em tempo real, as Audiências preditivas agora oferecem suporte à classificação em lote, ajudando a classificar usuários integrados e a ativar para destinos em lote.</li></ul> |
| [Relatórios de sobreposição](/help/using/reporting/dynamic-reports/dynamic-reports.md) | Atualizamos a metodologia que usamos para calcular os Relatórios de sobreposição ([Característica](/help/using/reporting/dynamic-reports/trait-trait-overlap-report.md), [Segmento para característica](/help/using/reporting/dynamic-reports/segment-trait-overlap-report.md), [Segmento para segmento](/help/using/reporting/dynamic-reports/segment-segment-overlap-report.md)). O cálculo dos Relatórios de sobreposição agora se baseia em assinaturas MinHash e relatórios (em vez de [1/54 amostragem de dados](/help/using/reporting/report-sampling.md#data-sampling-ratio)). |
...
| Tópico | Descrição |
|---- |----|
| [Acessibilidade no Audience Manager](../overview/accessibility-in-aam.md) | Acessibilidade refere-se a uma série de recursos que tornam um produto de software utilizável, com o menor esforço possível de usuários com várias deficiências, como visual, auditivo, cognitivo, motor ou outro tipo. Para facilitar o uso do Audience Manager para todos, desenvolvemos suporte para vários recursos de acessibilidade detalhados neste artigo. |
| [Nomes de arquivo S3 de entrada e ](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) nomes de arquivo FTP  [de entrada](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | Atualizamos as definições e fornecemos exemplos para os campos `DPID` e `DPID_TARGET_DATA_OWNER` na sintaxe do nome do arquivo. |

## Agosto de 2020 {#aam-august-updates}

| Tópico | Descrição |
|---- |----|
| [Correspondência de clientes do Google para destinos baseados em pessoas](../features/destinations/people-based-destinations-prerequisites.md#gcm) | Agora você pode usar [!UICONTROL People-Based Destinations] para público alvo de usuários via [!DNL Google Customer Match]. Antes de usar [!UICONTROL People-Based Destinations] para enviar seus segmentos de audiência primários para um destino [!DNL Google Customer Match], é obrigatório que [!DNL Google] o adicione à lista de permissões. Consulte a documentação [Destinos baseados em pessoas](../features/destinations/people-based-destinations-prerequisites.md#gcm) para obter detalhes. |
| [Novos servidores SFTP](../faq/faq-data-collection.md) | Agora você pode usar os novos servidores [!DNL SFTP] ([!DNL ftp-in-gtw] e [!DNL ftp-out-gtw]) que usam [!DNL AWS SFTP Gateway] em [!DNL S3]. |
...
| Tópico | Descrição |
|---- |----|
| [Compartilhamento de públicos entre o Audience Manager e a Adobe Experience Platform](/help/using/integration/integration-aep/aam-aep-audience-sharing.md#aep-aam-segment-population-differences) | A nova seção, **Entender as diferenças de população do segmento entre Audience Manager e Experience Platform**, descreve as possíveis razões pelas quais os números de população podem variar entre Experience Platform e Audience Manager. |
| [Introdução às APIs REST do Audience Manager](/help/using/api/rest-api-main/aam-api-getting-started.md#jwt) | Adicionamos uma nova seção, **Autenticação JWT (Conta de Serviço) usando[!DNL Adobe I/O]**. Consulte esta seção para autenticar nas APIs do Audience Manager usando [!DNL Adobe I/O]. |
